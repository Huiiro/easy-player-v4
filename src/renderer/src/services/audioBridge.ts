...
export const audioBridge = {
  ...
  async setReplayGain(config: {
    mode: 'off' | 'track' | 'album'
    preventClipping: boolean
  }): Promise<boolean> {
    return (await window.api.audio.setReplayGain(config)).success
  },
  async getReplayGain(): Promise<{
    mode: 'off' | 'track' | 'album'
    preventClipping: boolean
    active: boolean
    appliedGainDb: number
  } | null> {
    return cmd(await window.api.audio.getReplayGain())
  },
  async setPlaybackSpeed(config: { enabled: boolean; speed: number }): Promise<boolean> {
    return (await window.api.audio.setPlaybackSpeed(config)).success
  },
  async getPlaybackSpeed(): Promise<{ enabled: boolean; speed: number } | null> {
    return cmd(await window.api.audio.getPlaybackSpeed())
  },
  ...
  async setDelayConfig(config: DelayConfig): Promise<boolean> {
    return (await window.api.audio.setDelayConfig(config)).success
  },
  async getDelayConfig(): Promise<DelayConfig | null> {
    return cmd<DelayConfig>(await window.api.audio.getDelayConfig())
  },
  async setReverbConfig(config: {
    roomSize: number
    decay: number
    mix: number
  }): Promise<boolean> {
    return (await window.api.audio.setReverbConfig(config)).success
  },
  async getReverbConfig(): Promise<{ roomSize: number; decay: number; mix: number } | null> {
    return cmd(await window.api.audio.getReverbConfig())
  },
  async setChorusConfig(config: ChorusConfig): Promise<boolean> {
    return (await window.api.audio.setChorusConfig(config)).success
  },
  async getChorusConfig(): Promise<ChorusConfig | null> {
    return cmd<ChorusConfig>(await window.api.audio.getChorusConfig())
  },
  async setNoiseGateConfig(config: NoiseGateConfig): Promise<boolean> {
    return (await window.api.audio.setNoiseGateConfig(config)).success
  },
  async getNoiseGateConfig(): Promise<NoiseGateConfig | null> {
    return cmd<NoiseGateConfig>(await window.api.audio.getNoiseGateConfig())
  },
  async setPhaserConfig(config: PhaserConfig): Promise<boolean> {
    return (await window.api.audio.setPhaserConfig(config)).success
  },
  async getPhaserConfig(): Promise<PhaserConfig | null> {
    return cmd<PhaserConfig>(await window.api.audio.getPhaserConfig())
  },
  async setChannelMatrixConfig(config: ChannelMatrixConfig): Promise<boolean> {
    return (await window.api.audio.setChannelMatrixConfig(config)).success
  },
  async getChannelMatrixConfig(): Promise<ChannelMatrixConfig | null> {
    return cmd<ChannelMatrixConfig>(await window.api.audio.getChannelMatrixConfig())
  },
  async setLimiter(config: {
    enabled: boolean
    ceilingDb: number
    releaseMs: number
  }): Promise<boolean> {
    return (await window.api.audio.setLimiter(config)).success
  },
  async getLimiter(): Promise<{ enabled: boolean; ceilingDb: number; releaseMs: number } | null> {
    return cmd(await window.api.audio.getLimiter())
  },
  ...
  async getEngineInfo(): Promise<{
    version: string
    outputDevice: { backend: string; deviceId: string }
  } | null> {
  ...
  async setDopEnabled(enabled: boolean): Promise<boolean> {
    return (await window.api.audio.setDopEnabled(enabled)).success
  },
